...
class main():
  ...
  def switchtheme(num):
    ...
    mydict = json.dumps({ k : v for k,v in output_json.iteritems() if k != ch })
    ...
    for majorkey, subdict in output_json.iteritems():
      if re.match("(.*)(E|e)xtensions(.*)", majorkey):
        for subkey, value in subdict.iteritems():
          if re.match("(.*)(S|s)ettings(.*)", subkey):
            for ext, value in value.iteritems():
              if re.match(ch, ext):
